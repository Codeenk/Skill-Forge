...
def cmd_lint(args):
    ...
    is_forge_gen = isinstance(fm.get("metadata"), dict) and fm["metadata"].get("generator") == "skill-forge"
    if not os.path.isfile(sidecar):
        if is_forge_gen:
            errors.append(".forge/manifest.json sidecar missing on generator-produced bundle")
        else:
            warns.append("no .forge/manifest.json sidecar (drift verification unavailable)")
    else:
